...
class Player:
    ...
    def __init__(self, email, password, additional_indent, world_id="no_data", header_info=None):
    ...
    def send_spy(self, destination_habitat, source_habitat, cooper_amount):
        url = Data.server_url + "SpyAction/startSpyingTransit"
        params = {
            "copperAmount": str(cooper_amount),
            "sourceHabitatID": str(source_habitat,id),
            "destinationHabitatID": str(destination_habitat.id)
        }
        return self.repeated_request(url, params, times=3)
